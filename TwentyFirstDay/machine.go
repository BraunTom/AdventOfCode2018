...
func (m *machine) execute(instructions []instruction) {
	...
	for m.ip() >= 0 && m.ip() < len(instructions) {
		...
		if m.register[4] == 28 {
			fmt.Println(m.register) // breakpoint here
			for _, v := range possibleAs {
				if v == m.register[1] {
					fmt.Println(possibleAs[len(possibleAs)-1]) // solution for part 2
					break
				}
			}

			possibleAs = append(possibleAs, m.register[1])
		}
		m.incrementIp()
		i++
	}
